document.addEventListener('DOMContentLoaded', () => {
    ...
    function getQuickNutritionFact() {
        const facts = [
            "🍎 An apple contains about 95 calories and is a good source of fiber.",
            "🥦 Broccoli is rich in vitamins C and K, and is a good source of folate.",
            "🥚 Eggs are one of the most nutritious foods, containing a little bit of almost every nutrient you need.",
            "🥑 Avocados are loaded with heart-healthy monounsaturated fatty acids.",
            "🍓 Strawberries are among the most antioxidant-rich fruits and are particularly high in vitamin C.",
            "🥛 Milk is a great source of calcium, vitamin D, and protein.",
            "🍗 Chicken breast is high in protein and low in fat if eaten without the skin.",
            "🐟 Fatty fish like salmon are loaded with omega-3 fatty acids, crucial for brain health.",
            "🥜 Almonds are high in healthy fats, vitamin E, and magnesium.",
            "🍠 Sweet potatoes are a great source of beta-carotene, vitamin C, and potassium."
        ];
    ...
    async function sendMessage() {
        ...
        try {
            ...
            if (message.toLowerCase() === 'help') {
                ...
                // Show help message
                addMessage('🤖 **Chat Assistant Help**\n\n' +
                          '• Ask about nutrition facts for any food\n' +
                          '• Get healthy eating recommendations\n' +
                          '• Learn about ingredients or allergens\n' +
                          '• Ask for meal planning advice\n' +
                          '• Get explanations about nutrition terms\n' +
                          '• Type "fact" for a random nutrition fact\n' +
            ...
            if (currentProduct && Math.random() > 0.7) {
                setTimeout(() => {
                    ...
                    addMessage(`💡 You might also want to ask: "${randomSuggestion}"`); 
                }, 1000);
            }
    ...
    function displayProductInfo(product) {
        ...
        const nutritionGradeBar = document.getElementById('nutrition-grade-bar');
        ...
        if (product.nutrition_grade_fr) {
            ...
            // Set the bar color based on the grade
            // Set the progress width directly on the element itself
            const progressWidth = gradePercentages[grade] || '0%';
            nutritionGradeBar.style.width = progressWidth;
            nutritionGradeBar.className = 'nutrition-grade-bar grade-' + product.nutrition_grade_fr.toLowerCase();
            
            // Add a tooltip with the English description
            if (gradeDescriptions[grade]) {
                nutritionGradeElement.textContent = grade;
                nutritionGradeElement.title = gradeDescriptions[grade];
                nutritionGradeBar.title = gradeDescriptions[grade] + ' - Score: ' + score + '/100';
            ...
            .then(data => {
                if (data.grade) {
                    ...
                    const progressWidth = gradePercentages[grade] || '0%';
                    nutritionGradeBar.style.width = progressWidth;
                    nutritionGradeBar.className = 'nutrition-grade-bar grade-' + data.grade.toLowerCase();
        ...
        try {
            ...
            scanHistory.forEach(item => {
                ...
                historyItem.innerHTML = `
                    ...
                    <button class="history-item-expand">▼</button>
    ...
    function generateAISuggestion(product) {
        ...
        if (goodAspects.length > 0) {
            ...
            goodAspectsHTML = `
            <div class="ai-good">
                <span class="ai-good-icon">✨</span>
                <span class="ai-good-text">Good to know: ${selectedGoodAspects.join('. ')}</span>
            </div>`;
        }
        
        // Create the AI suggestion HTML
        const suggestionHTML = verdict === 'Avoid' ? `
            <div class="ai-verdict ${verdictClass}">⚠️ ✕ Avoid: This food is high in fat</div>
            <div class="ai-reason">Why: This product has a D - Poor nutritional quality.</div>
            <div class="ai-tip">
